...
const makeAfficherUnJoueur = (joueurRepository: JoueurRepository): AfficherLeJoueur => async (
    id: string,
): Promise<Joueur> => {
    return (await joueurRepository.findJoueurById(id)).orElseThrow(
        () => new JoueurInexistantError(id),
    )
}
